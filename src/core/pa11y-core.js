...
export default async function runPa11y([...pages]) {
    ...
    for (const folder of existsFolder) {
        ...
        for (const url of urls) {
            ...
            try {
                ...
                const reportTemplate = readFileSync(join(__dirname, '..', 'views', 'report-template.ejs'), 'utf8');
    ...
    const finalHtml = await generateFinalHtml(pageResults, testCounter);
    writeFileSync(`./reports/final_report_${testCounter}.html`, finalHtml);
    testCounter++;
    return finalHtml;
}
...
async function generateFinalHtml(pageResults, testCounter) {
    const renderedResults = pageResults.map((pageResult) => {
        ...
        if (results && results.issues) {
            ...
            const creationDate = moment().format("DD-mm-YYYY HH:mm");
            const renderedHtml = render(template, { pageTitle, creationDate, url, errors, warnings, notices, hljs, pages: pageResults });
            return renderedHtml;
        } else {
    ...
    if (testCounter === 1) {
        ...
        const previousTestCounter = testCounter - 1;
        const previousHtmlPath = `./reports/final_report_${previousTestCounter}.html`;

        if (fs.existsSync(previousHtmlPath)) {
            const previousHtml = readFileSync(previousHtmlPath, 'utf8');
            return previousHtml + finalHtml;
        } else {
            return finalHtml;
        }
    }
